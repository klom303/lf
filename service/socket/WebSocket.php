...
class WebSocket
{
    ...
    {
        $config = include __CONFIG__ . '/socket.php';
        $a = isset($config['host'])?$config['host']:null;
        $p = isset($config['port'])?$config['port']:null;
    ...
    public function run()
    {
        while(true)
        {
            ...
            $write = NULL;
            $except = NULL;
            socket_select($socketArr, $write, $except, NULL);  //select the socket with message automatically
    ...
    public function multiSend($client,$msg)
    {
        $this->say("> Resource id:" . substr('' . $client, 13) . ' ' . $msg);
        $msg = $this->wrap($msg);
        foreach ($this->users as $user) {
            if($user->socket!=$client && $user->handshake){
